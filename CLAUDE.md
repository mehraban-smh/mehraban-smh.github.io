...
- UNLISTED manuscripts shared by direct link only, in the root: `ai-mould-review.pdf` (https://mehraban.uk/ai-mould-review.pdf) and `retrofit-mould-twin.pdf` (https://mehraban.uk/retrofit-mould-twin.pdf). Never link to them from the site, never rename or delete them, and keep `robots.txt` (which disallows each). Add any future unlisted file the same way: root-level, short descriptive name, one `Disallow` line in `robots.txt`.
...
- In `<head>`: the `google-site-verification` meta tag (Search Console ownership) and `<link rel="canonical" href="https://mehraban.uk/">`. In the JSON-LD: the `alternateName` list of name variants incl. Persian script. In the footer: the small grey line with the full name and Persian script (`.foot .aka`); the owner wants it unobtrusive but it must stay VISIBLE (never white-on-white or display:none, which Google treats as hidden text). `sitemap.xml` (homepage only, by the owner's choice) and the `Sitemap:` line in `robots.txt`.
