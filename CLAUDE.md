...
- In `<head>`: the `google-site-verification` meta tag (Search Console ownership) and `<link rel="canonical" href="https://mehraban.uk/">`. In the JSON-LD: the `alternateName` list of name variants incl. Persian script. In About: the "My full name is …" sentence (`p.aka`). `sitemap.xml` (homepage only, by the owner's choice) and the `Sitemap:` line in `robots.txt`.
