...
- `ai-mould-review.pdf` in the root is an UNLISTED manuscript shared by direct link only (https://mehraban.uk/ai-mould-review.pdf). Never link to it from the site, never rename or delete it, and keep `robots.txt` (which disallows it). Add any future unlisted file the same way: root-level, short descriptive name, one `Disallow` line in `robots.txt`.
